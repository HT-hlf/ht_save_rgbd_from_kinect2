...
string topic1_name = "/kinect2/qhd/image_color_rect"; //topic 名称
string topic2_name = "/kinect2/qhd/image_depth_rect";

string filename_rgbdata="/home/nvidia/recordData/RGBD/rgbdata.txt";
string filename_depthdata="/home/nvidia/recordData/RGBD/depthdata.txt";
string save_imagedata = "/home/nvidia/recordData/RGBD/";
...
void  callback_function_depth(const sensor_msgs::Image::ConstPtr  image_data)
{
   ...
   cv_bridge::CvImageConstPtr pCvImage;// 声明一个CvImage指针的实例
   pCvImage = cv_bridge::toCvShare(image_data, image_data->encoding);//将ROS消息中的图象信息提取，生成新cv类型的图象，复制给CvImage指针
   pCvImage->image.copyTo(temp);
//cout<<"the  depth"<<image_data->data<<endl;
